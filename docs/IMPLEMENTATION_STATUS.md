...
| Packaging and guides | package manifests, `LICENSE`, package READMEs, `docs/guides`, `docs/DEPLOYMENT.md`, `tests/install`, `docs/RELEASE_CHECKLIST.md` | MIT license; one version for all five public packages; published manifests pin internal dependencies to that version, drop the in-repository `streamotter-source` condition, and carry `main` and `types`; each package's README is its npm page; the workbench ships the license notices of the Socket.IO client code it bundles. The guides cover getting started, adding StreamOtter to an existing app, Kafka, production, and troubleshooting. The getting-started flow was run end to end with the published packages and Vite 8.3.1 in Chromium: install, `init`, `dev`, a page subscribed with a development token, a live update, `stale` when the gateway stopped, and `resync-required` after a restart until the page was reloaded. |
...
- **Release candidates on npm.** `0.1.0-rc.1` was published on September 25 from the `v0.1.0-rc.1` tag. Because the packages were new, npm pointed both `next` and `latest` at it. `0.1.0-rc.2`, which changes only documentation, manifest metadata, and tests, was published the same day with `--tag latest`, so the npm pages show its READMEs. `next` still points to `0.1.0-rc.1`: with passkey two-factor authentication, each `npm dist-tag add` needs a browser sign-in, and five in quick succession hit npm's rate limit (`E429`). The step is optional; see the [release checklist](./RELEASE_CHECKLIST.md). `0.1.0-rc.3`, which adds the all-in-one `streamotter` package (owned by the publishing account; the other five belong to the `streamotter` organization) and the CLI's import detection, was published the same way that day. For about 20 seconds after publishing, the registry served `@streamotter/cli@0.1.0-rc.3` as not found while `streamotter@0.1.0-rc.3` was already live, so wait a minute before verifying from the registry.
