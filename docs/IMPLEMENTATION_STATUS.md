...
September 25, 2026 · V1 release candidate: `0.1.0-rc.1` published to npm; `0.1.0-rc.2` (documentation and packaging) prepared
...
| `STREAMOTTER_INSTALL_FROM=registry pnpm test:install` | **14 / 14 passed** against the published `0.1.0-rc.1` from the npm registry (broker running, Node 26.9.0), after the owner published on September 25. |
...
- **Release candidates on npm.** `0.1.0-rc.1` was published on September 25 from the `v0.1.0-rc.1` tag. Because the packages were new, npm pointed both `next` and `latest` at it. `0.1.0-rc.2` changes only documentation, manifest metadata, and tests; until the first stable version, release candidates are published with both tags ([release checklist](./RELEASE_CHECKLIST.md)).
