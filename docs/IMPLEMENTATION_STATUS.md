...
- **Release candidates on npm.** `0.1.0-rc.1` was published on September 25 from the `v0.1.0-rc.1` tag. Because the packages were new, npm pointed both `next` and `latest` at it. `0.1.0-rc.2`, which changes only documentation, manifest metadata, and tests, was published the same day with `--tag latest`, so the npm pages show its READMEs. `next` still points to `0.1.0-rc.1`: with the owner's passkey-only two-factor authentication, `npm dist-tag add` waited without completing (see the [release checklist](./RELEASE_CHECKLIST.md)).
