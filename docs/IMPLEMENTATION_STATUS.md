...
September 25, 2026 · V1 release candidate `0.1.0-rc.1`, published to npm under the `next` tag
...
- **Release candidate on npm.** `0.1.0-rc.1` was published on September 25, from the `v0.1.0-rc.1` tag, under the `next` dist-tag. Because the packages are new, npm also points `latest` at it until a stable version is published. Later releases follow the [release checklist](./RELEASE_CHECKLIST.md).
- **An intermittent Kafka crash test (open).** On September 25, `tests/kafka/03-crash.test.ts` failed once. The replacement gateway did not join the consumer group within its 30-second startup deadline while the killed member's 30-second session expired. The test file then hung until it was stopped. A rerun after restarting the broker passed 20 / 20, and CI's `extended.yml` passed. The test needs to avoid that boundary and to fail fast.
