...
- **Release candidates on npm.** `0.1.0-rc.1` was published on September 25 from the `v0.1.0-rc.1` tag. Because the packages were new, npm pointed both `next` and `latest` at it. `0.1.0-rc.2`, which changes only documentation, manifest metadata, and tests, was published the same day with `--tag latest`, so the npm pages show its READMEs. `next` still points to `0.1.0-rc.1`: with passkey two-factor authentication, each `npm dist-tag add` needs a browser sign-in, and five in quick succession hit npm's rate limit (`E429`). The step is optional; see the [release checklist](./RELEASE_CHECKLIST.md).
